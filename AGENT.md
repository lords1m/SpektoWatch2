...
`agent/screenshots/` is the local screenshot workspace generated by
`ScreenshotCatalogTests` and `WatchAppScreenshotTests`. PNG output is ignored
by default and can be published as a CI artifact. Read
`agent/screenshots/INDEX.md` when a refreshed local catalog is present.
